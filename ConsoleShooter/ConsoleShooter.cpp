...
uint64_t g_weyl = 0xb5ad4eceda1ce2a9;

uint32_t rand(uint32_t seed) {
	static uint64_t x;
	x *= x;
	x += (g_weyl += 0xb5ad4eceda1ce2a9);
	return (((x >> 32) | (x & 0xFFFFFFFF)) / 65536) % 32768;
}
...
int main()
{
	...
	wchar_t* screen = new wchar_t[nScreenWidth * nScreenHeight + 1000];
	...
	while (1)
	{
		...
		for (int x = 0; x < nScreenWidth; x++)
		{
			...
			for (int y = 0; y < nScreenHeight; y++)
			{
				...
				short nShade = ' ';
				if		(fDistanceToWall + ((float)rand(x * nScreenWidth + y) / (float)0xffff * 3.0) <= fDepth / 4.0f)		nShade = 0x2588;	// Very close	
				else if (fDistanceToWall + ((float)rand(x * nScreenWidth + y) / (float)0xffff * 3.0) < fDepth / 3.0f)		nShade = 0x2593;
				else if (fDistanceToWall + ((float)rand(x * nScreenWidth + y) / (float)0xffff * 3.0) < fDepth / 2.0f)		nShade = 0x2592;
				else if (fDistanceToWall + ((float)rand(x * nScreenWidth + y) / (float)0xffff * 3.0) < fDepth)				nShade = 0x2591;
